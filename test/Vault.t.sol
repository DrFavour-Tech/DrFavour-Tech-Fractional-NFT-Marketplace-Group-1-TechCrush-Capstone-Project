...
contract VaultTest is Test {
    ...
    function test_redeemNFT_happyPath_unlocksAssetCorrectly() public {
        ...
        vm.prank(user);
        fractionToken.transfer(buyer, totalFractionsRaw);

        // Verifying buyer holds 100% of supply before redemption
        assertEq(fractionToken.balanceOf(buyer), totalFractionsRaw);
        ...
        // 4. Assertions
        assertEq(nft.ownerOf(tokenId), buyer); // Buyer now owns the physical NFT
    ...
    function test_redeemNFT_revertsOnInsufficientFractions() public {
        ...
        // User transfers almost everything to Buyer, but holds back 1 token base unit (1 wei of ERC20)
        uint256 totalFractionsRaw = fractionSupply * 1e18;
        vm.prank(user);
        fractionToken.transfer(buyer, totalFractionsRaw - 1);

        // Buyer tries to redeem with 99.9999...% supply
        vm.prank(buyer);
